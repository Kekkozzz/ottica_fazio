...
export default function LiquidGlassNavbar() {
    ...
    const [isDesktop1500, setIsDesktop1500] = useState(false); // >=1500px
    ...
    useEffect(() => {
        ...
        // Breakpoint custom 1500px
        const mql = window.matchMedia('(min-width: 1500px)');
        const updateMatch = (e) => setIsDesktop1500(e.matches);
        updateMatch(mql);
    ...
    return (
        <>
            {/* Logo */}
            <div className="fixed top-4 left-4 sm:left-6 xl:left-10 z-50 flex items-center gap-2 sm:gap-3 md:gap-4 cursor-pointer"
                onClick={() => router.push('/')}
            >
                <button className="focus:outline-none" aria-label="Vai alla Home" type="button">
                    <Image
                        ...
                        height={40}
                        priority
                        className="drop-shadow-lg sm:w-[48px] sm:h-[48px] xl:w-[52px] xl:h-[52px]"
                    />
                </button>
                <span className="hidden sm:inline-block font-josefin font-extrabold text-base sm:text-lg xl:text-2xl tracking-tight text-[#2d3142] drop-shadow-sm select-none" style={{ letterSpacing: '0.04em' }}>
                    OTTICA FAZIO
                </span>
            </div>

            {/* Navbar desktop (>=1500px) */}
            {isDesktop1500 && (
                                    ...
                                    return (
                                        <div key={index} className="relative select-none">
                                            <button
                                                ...
                                                className={`main-nav-button group relative flex items-center gap-2 px-6 py-2 text-base font-bold text-white transition-all transition-colors duration-300 ease-out overflow-hidden rounded-full cursor-pointer shadow-lg bg-gradient-to-br from-black to-gray-800 hover:from-red-600 hover:to-red-700 hover:shadow-xl ${hasDropdown ? 'pr-10' : ''}`}
                                                style={{ minHeight: 44 }}
                                            ...
                                            >
                                                ...
                                                {hasDropdown && (
                                                    <>
                                                        <span className={`cursor-pointer absolute right-3 top-1/2 -translate-y-1/2 transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`}>
                                                            <ChevronDown size={18} />
                                                        </span>
            ...
            {/* Mobile / hamburger (<1500px) */}
            <div className={`fixed top-4 right-4 z-50 ${isDesktop1500 ? 'hidden' : ''}`}>
                <button
                                                ...
                                                <div className="pt-2 space-y-1">
                                                    {item.dropdown.map((dropdownItem, dropdownIndex) => (
                                                        <button
                                                            ...
                                                            className="cursor-pointer flex items-center w-full text-left px-4 py-3 text-sm font-medium text-gray-700 bg-gray-50 hover:bg-red-500 hover:text-white transition-all duration-200 rounded-lg border-l-4 border-red-500 hover:border-red-600 hover:translate-x-1 ml-2 mr-2"
                                                        >
                                                            <span className="inline-block w-2 h-2 rounded-full bg-red-500 mr-3"></span>
                                                            {dropdownItem.label}
                                                        </button>
